...
if (form) {
    form.addEventListener("submit", e => {
        ...
        document.getElementById("m_checkin").textContent = new Date(checkin).toLocaleDateString('en-US', { 
            year: 'numeric', 
            month: 'long', 
            day: 'numeric' 
        });
        document.getElementById("m_checkout").textContent = new Date(checkout).toLocaleDateString('en-US', { 
            year: 'numeric', 
            month: 'long', 
            day: 'numeric' 
        });
        document.getElementById("m_guests").textContent = guests + (guests > 1 ? " guests" : " guest");
        document.getElementById("m_time").textContent = checkinTime;
        document.getElementById("m_nights_text").textContent = `₱${pricePerNight.toLocaleString()} × ${nights} night${nights > 1 ? 's' : ''}`;
        ...
        document.getElementById("m_extra_night").textContent = `₱${extraNight.toLocaleString()}`;
    ...
    if (confirmSubmit) {
        confirmSubmit.addEventListener("click", () => {
            ...
            confirmSubmit.innerHTML = '<i class="bx bx-loader-alt bx-spin"></i> Processing...';
