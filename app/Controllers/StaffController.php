...
class StaffController
{
    ...
    public function confirm()
    {
        ...
        error_log("=== STAFF CONFIRM BOOKING #{$id} ===");
        ...
        try {
            ...
            header("Location: /Hotel_Reservation_System/app/public/index.php?controller=staff&action=index&{$successParam}");
            exit();
        } catch (Exception $e) {
            $this->db->rollBack();
            error_log("❌ Confirm error: " . $e->getMessage());
            error_log($e->getTraceAsString());
            header("Location: /Hotel_Reservation_System/app/public/index.php?controller=staff&action=index&error=confirm_failed");
            exit();
        }
    }
    ...
    public function restore()
    {
        ...
        // true is passed to getBookingById to include deleted records
        $booking = $this->bookingModel->getBookingById($id, true);

        if ($booking) {
            $this->bookingModel->restore($id);

            // If the booking was confirmed before deletion, set the room back to booked
            if (strtolower($booking['booking_status']) === 'confirmed') {
                $this->roomModel->updateAvailability($booking['RoomID'], 'booked');
            }
        ...
        header("Location: /Hotel_Reservation_System/app/public/index.php?controller=staff&action=history&success=restored");
        exit();
    }
    ...
    /**
     * Reservations page (Confirmed/Pending bookings NOT checked in)
     */

    public function reservations()
    {
        // Add debugging
        error_log("=== STAFF RESERVATIONS PAGE LOADED ===");
    ...
    public function updateReservation()
    {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            header("Location: /Hotel_Reservation_System/app/public/index.php?controller=staff&action=reservations&error=invalid_request");
            exit();
        }
        ...
        ";
            ...
            header("Location: /Hotel_Reservation_System/app/public/index.php?controller=staff&action=reservations&error=update_failed");
            exit();
        }
    }
    ...
    public function checkinReservation()
    {
        ...
        ";
            ...
            if (!$booking) {
                ...
                $this->db->rollBack();
                header("Location: /Hotel_Reservation_System/app/public/index.php?controller=staff&action=reservations&error=booking_not_found");
                exit();
            }
            ...
            if ($existingGuest) {
                ...
                $this->db->rollBack();
                header("Location: /Hotel_Reservation_System/app/public/index.php?controller=staff&action=reservations&error=already_checked_in");
                exit();
            }
        ...
        ";
            ...
            error_log($e->getTraceAsString());
            header("Location: /Hotel_Reservation_System/app/public/index.php?controller=staff&action=reservations&error=checkin_failed&msg=" . urlencode($e->getMessage()));
            exit();
        }
    }
    ...
    ";
        ...
        include __DIR__ . '/../Views/staff/current_guest.php';
    }

    /**
     * Checks out a guest, archives guest data, frees the room, and updates booking status.
     */
    ...
    private function getValue($sql)
    {
        try {
            ...
            error_log("getValue SQL Error: " . $e->getMessage() . " | Query: " . $sql);
            return 0;
        }
    }
}
